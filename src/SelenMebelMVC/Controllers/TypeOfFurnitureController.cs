    ...
    public class TypeOfFurnitureController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchBy, string searchValue, int pageIndex = 1, int pageSize = 10)
        {
            try
            {
                ...
                {
                    ViewBag.Furnitures = typeOfFurnitures.OrderByDescending(f => f.CreatedAt);
                    ...
                    else
                    {
                        ...
                        for (int i = 1; i <= totalPages; i++)
                        {
                            ...
                            var result = typeOfFurnitures.OrderByDescending(f => f.CreatedAt);
        ...
        public async Task<ViewResult> Edit(long id)
        {
            ...
            var category = await _categoryService.RetrieveByIdAsync(typeOfFurniture.Category.Id);
            if (typeOfFurniture is not null)
            {

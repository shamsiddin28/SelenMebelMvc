...
namespace SelenMebel.Service.Services.Admins
{
	public class AdminService : IAdminService
	{
		...
		public async Task<bool> DeleteAsync(long id)
		{
			...
			var isDeletedImage = await _fileService.DeleteImageAsync(admin.Image);
			return await _unitOfWork.Admins.DeleteAsync(id) && isDeletedImage;
		}
		...
		public async Task<AdminViewModel> GetByTokenAsync()
		{
			...
			var result = (AdminViewModel)admin;
			return result;
		}

		public async Task<AdminViewModel> GetByPhoneNumberAsync(string phoneNumber)
		{
			var admin = await _unitOfWork.Admins.FirstOrDefault(x => x.PhoneNumber == phoneNumber || x.PhoneNumber.ToLower().Contains(phoneNumber.ToLower()));
